---
type: concept
title: "Competitor Pricing Benchmarks"
source_count: 1
confidence: medium
last_updated: 2026-06-19
tags: [competitive, pricing, sales-motion]
---

## Definition
Consolidated, directional benchmarks of what Anduin's competitors charge, drawn from field intel (deal quotes, ex-employee reports, customer disclosures). These are **individual data points, not list prices** — useful for framing and negotiation, not as guaranteed competitor pricing.

## Key dynamics
- **Passthrough's one-time, non-recurring per-fund fee is the single most disruptive pricing lever** in the market — it reframes the buy as a one-time cost "good for the life of the fund," repeatedly beating Anduin's recurring model in emerging/mid deals.
- **Subscribe bundles managed services + API into a flat platform fee**, collapsing Anduin's itemized quote into one low number.
- **Side-letter pricing (Ontra) is AUM/volume-based and high** ($75–80K/yr) — a different pricing axis from per-fund Fundsub.
- **AML/KYC is increasingly priced per-investor/entity** ($75–100), a recurring attach competitors use to raise stickiness and ACV.

## Evidence & examples
**Fundsub — per fund / per deal (directional):**
| Competitor | Quote | Context |
|---|---|---|
| [[competitors/passthrough]] | $5–7.5K | emerging fund (InVivium) |
| Passthrough | $10K + $7K OFAC | emerging fund, Jan 2023 |
| Passthrough | $7–15K / $15–20K per fundraise | general 2023–24 |
| Passthrough | $18K one-time (3 vehicles) | NYCA, vs Anduin $61.5K |
| Passthrough | $22K one-time | Centre Lane |
| Passthrough | $40K + $7.5K impl → $20K annual | Blue Torch ($7.2B) |
| Passthrough | $11,500 all-in subdocs + $275/LP KYC; $250/LP managed review | Raine ($3B), 2026 |
| [[competitors/atominvest]] | $10K setup + $5–10K/yr | per fund, modular |
| AtomInvest | $20K (Fundsub + 1 fund) | Indico; also $20K vs Anduin $60K |
| AtomInvest | $45K + $7.5K impl → $28.4K annual | Blue Torch (enterprise) |
| [[competitors/subscribe]] | $45K all-in platform fee | incl. managed services + API + 3-mo trial |

**AML/KYC:**
- Passthrough: $75/entity check (2023); ~$100/yr/investor (2025); $275–250/LP managed (2026).

**Side letter / terms tracking:**
- [[competitors/ontra]]: ~$75–80K/yr (priced on AUM, # docs, complexity).

**Anduin reference (from pricing FAQ):** Emerging $6K/fund/yr · Established $8K · Enterprise $15K/fund/yr — plus itemized data import, data room, API, OCR, KYC/AML lines.

## Tensions & open questions
- Many figures are 2022–2024 and may be stale; competitor pricing has trended toward bundling and managed services.
- Anduin's itemized quoting consistently produces a much larger headline number than competitors' bundled flat fees (e.g., a Subscribe deal: ~$45K vs. Anduin ~$176K). Open question: when to lead with a bundle to neutralize this.
- Competitor one-time-fee models vs. Anduin recurring: how to defend recurring value (CS, maturity, updates) at the emerging tier.

## Related
- [[concepts/competitive-win-loss]]
- [[concepts/product-packaging-bundling]]
- [[competitors/passthrough]] · [[competitors/atominvest]] · [[competitors/subscribe]] · [[competitors/ontra]]
- [[sources/competitive-intel-tracking-table]]
