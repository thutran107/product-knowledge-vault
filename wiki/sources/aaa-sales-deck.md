...
- **Advisor tags**: GPs communicate custom statuses ("Custodian approve," "pending custodian approval") or notes to advisors via the Advisor's tags column; tags sync to the advisor's dashboard in Advisor Advantage. (See [[wiki/sources/aaa-advisor-tags|AAA Advisor Tags release note]].)
...
## Connections
- Primary capability source for [[wiki/products/aaa|AAA (Advisor Advantage)]].
- Assign/unassign capability corroborates the existing [[wiki/sources/aaa-advisor-subscription-assign|Assign/Unassign Subscriptions to Advisor Entities]] release note.
- Signature/custodian elements connect to [[wiki/products/e-signature|E-signature]] and the Schwab workflow ([[wiki/sources/esignature-schwab-implementation-guide|Schwab Signature setup]]).
- Auto-fill via investor passport connects to [[wiki/products/investor-access|Investor Access]].
