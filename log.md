...
## [2026-06-30] lint | Full vault lint — 1 finding fixed (fragile pages)

**Operation:** lint
**Summary:** User-requested full §7 lint across all 186 wiki pages. Result: vault is healthy. **Critical** — 8 broken wikilinks, but all 8 are inside `log.md` (append-only journal): 4 legacy `wiki/`-prefixed refs in historical entries, 3 literal `[[...]]`/`[[wiki/...]]` prose ellipses, and 1 self-quoted bad slug from the prior lint entry; the knowledge graph (wiki/ + index.md) has **0** broken links. Left untouched per §4 (never edit existing log entries). **High** — 0 orphans, 0 index gaps, 0 missing entity pages. **Medium** — 0 frontmatter errors, 0 contested, 0 source-drift (no sha256 in raw; N/A). **Low** — fixed: 12 fragile pages (`source_count: 1`, no `confidence`) given explicit ratings — 8 feature pages + Neuberger Berman + Accolade + NXSTEP → `confidence: medium` (uncontested how-it-works / case-study sources), KKR → `confidence: low` (thin single SSO mention). 11 `confidence: low` competitor pages are by-design (single-source intel; no action). Data gaps unchanged (already tracked in overview). **Style** — 1 page >200 lines (query-answering-karpathy-method.md, 303; not split per user), 0 missing feature↔product cross-refs, tag taxonomy clean (0 hash-prefixed; a few near-duplicate tags noted but not merged: workflow/workflows, gtm/gtm-training, asset-management/asset-manager, cs-process/cs-playbook). Log at 41 entries (rotate threshold 500). Post-fix fragile count: **0**.
**Pages touched:** wiki/features/{idm-clients-tab,idm-contacts,idm-firm-settings,idm-funds-tab,idm-public-api,investor-portal-communications,investor-portal-profile-sharing,platform-advanced-white-labeling}.md, wiki/customers/{accolade,kkr,neuberger-berman,nxstep}.md (added confidence), log.md
